...
app.post('/values', async (c) => {
  try {
    ...
    const { category_id, label, sort_order = 0 } = await c.req.json()

    if (!category_id || !label) {
      return c.json({ error: '필수 항목을 입력해주세요.' }, 400)
    }

    // 1. 카테고리 정보 조회 (name 가져오기)
    ...
    const categoryName = categoryResult.name as string

    // 2. Prefix 및 Sequence 조회
    const prefixResult = await c.env.DB.prepare(`
      SELECT prefix, next_sequence FROM item_category_prefixes WHERE category_name = ?
    `).bind(categoryName).first()

    if (!prefixResult) {
      return c.json({ error: '카테고리 Prefix를 찾을 수 없습니다.' }, 404)
    }

    const prefix = prefixResult.prefix as string
    const sequence = prefixResult.next_sequence as number

    // 3. 자동 코드 생성: Prefix_XXXX (4자리 일련번호)
    const autoGeneratedCode = `${prefix}_${String(sequence).padStart(4, '0')}`

    // 4. 항목 삽입
    const result = await c.env.DB.prepare(`
      INSERT INTO item_values (category_id, value, label, sort_order)
      VALUES (?, ?, ?, ?)
    `).bind(category_id, autoGeneratedCode, label, sort_order).run()

    // 5. Sequence 증가
    await c.env.DB.prepare(`
      UPDATE item_category_prefixes
